...
export function CategoryBreakdownBars({
  ...
  const source = rows
    ...
    .map((row) => ({
      ...row,
      label: row.parent ? `${row.parent} / ${row.category}` : row.category,
    }))
  ...
  const displayRows: DisplayRow[] = remainder.length
    ? [
        ...visible,
        {
          ...
          share: total
            ? remainder.reduce((sum, row) => sum + row.amount, 0) / total
            : 0,
          label: "其他",
          isOther: true,
        },
  ...
  return (
    <ul className="category-breakdown-bars" aria-label="分类支出排行">
      {displayRows.map((row, index) => {
          ...
          <>
            <div className="category-breakdown-row-head">
              ...
              <div className="category-breakdown-values">
                ...
                <Typography.Text
                  type="secondary"
                  className="category-breakdown-share"
                >
            ...
            <Progress
              className="category-breakdown-progress"
              percent={Number((row.share * 100).toFixed(1))}
              showInfo={false}
              strokeColor={
                index === 0 ? "#176b62" : row.isOther ? "#a9bbb4" : "#6d8f78"
              }
              trailColor="#e4eae5"
              size="small"
            />
        ...
        if (onCategoryClick && !row.isOther) {
          return (
            <li key={`${row.parent || "root"}-${row.category}`}>
              <button
                type="button"
                className="category-breakdown-row category-breakdown-row-button"
                onClick={() => onCategoryClick(row)}
                aria-label={`查看${row.label}账目`}
              >
                {content}
              </button>
            </li>
        ...
        return (
          <li
            key={`${row.parent || "root"}-${row.category}`}
