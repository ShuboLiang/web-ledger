...
import { Alert, Button, Card, Col, DatePicker, Drawer, Empty, Flex, Grid, List, Progress, Row, Segmented, Skeleton, Space, Statistic, Tag, Typography } from "antd";
import dayjs from "dayjs";
import { useEffect, useMemo, useState, type CSSProperties } from "react";
import { useNavigate, useSearchParams } from "react-router-dom";
import { api, type Breakdown, type BreakdownChange, type Dashboard, type Transaction } from "@/lib/api";
...
type RangeAnalytics = { start: string; end: string; days: number; cashflow: Dashboard["cashflow"]["month"]; breakdown: Breakdown[]; secondaryBreakdown: Breakdown[]; comparisonBreakdown: BreakdownChange[]; comparison: Dashboard["comparison"]["month"]; series: { key: string; label: string; amount: number }[] };
...
const isScope = (value: string | null): value is Scope => Boolean(value && scopes.includes(value as Scope));
const dateText = (value: string) => new Intl.DateTimeFormat("zh-CN", { year: "numeric", month: "2-digit", day: "2-digit" }).format(dayjs(value).toDate());
const rangeText = (range: [string, string]) => `${dateText(range[0])} — ${dateText(range[1])}`;
...
export function AnalyticsPage() {
  ...
  const customRange = params.get("start") && params.get("end") ? [params.get("start")!, params.get("end")!] as [string, string] : null;
  const setParam = (key: string, value: string) => setParams((current) => { const next = new URLSearchParams(current); value ? next.set(key, value) : next.delete(key); return next; });
  useEffect(() => {
    const saved = sessionStorage.getItem(analyticsFilterKey);
    if (!params.toString() && saved) { setParams(new URLSearchParams(saved), { replace: true }); return; }
    if (params.toString()) sessionStorage.setItem(analyticsFilterKey, `?${params.toString()}`);
  }, [params, setParams]);
  const setScope = (value: Scope) => { setParams((current) => { const next = new URLSearchParams(current); next.set("scope", value); next.delete("start"); next.delete("end"); return next; }); setCustomMode(false); };
  const setAnchor = (value: string) => setParam("anchor", value);
  const clearCustomRange = () => { setParams((current) => { const next = new URLSearchParams(current); next.delete("start"); next.delete("end"); return next; }); setCustomMode(false); };
  const { data, isLoading, isError, error, refetch } = useQuery({ queryKey: ["analytics", anchor], queryFn: () => api<Dashboard>(`/api/dashboard${anchor ? `?anchor=${anchor}` : ""}`) });
  const selected = data?.anchor || anchor || dayjs().format("YYYY-MM-DD");
  const calendarMonth = params.get("calendarMonth") || customRange?.[1].slice(0, 7) || selected.slice(0, 7);
  const requestedRange = customRange || (data ? data.ranges[scope] as [string, string] : null);
  const { data: customData, isLoading: customLoading, isError: customError, refetch: refetchCustom } = useQuery({ queryKey: ["analytics-range", customRange], enabled: Boolean(customRange), queryFn: () => api<RangeAnalytics>(`/api/dashboard/range?start=${customRange![0]}&end=${customRange![1]}`) });
  const { data: large, isLoading: largeLoading, isError: largeError, refetch: refetchLarge } = useQuery({ queryKey: ["large-expenses", requestedRange], enabled: Boolean(requestedRange), queryFn: () => api<{ records: Transaction[] }>(`/api/transactions?start=${requestedRange![0]}&end=${requestedRange![1]}&page=1&pageSize=5&direction=expense&sortBy=amount&sortOrder=asc`) });
  ...
  const { data: calendarData, isFetching: calendarLoading, isError: calendarError, refetch: refetchCalendar } = useQuery({ queryKey: ["expense-calendar", calendarStart, calendarEnd], enabled: calendarInRange, queryFn: () => api<{ series: { key: string; amount: number }[] }>(`/api/dashboard/range?start=${calendarStart}&end=${calendarEnd}`) });
  ...
  const { data: heatDay, isFetching: heatDayLoading, isError: heatDayError, refetch: refetchHeatDay } = useQuery({ queryKey: ["heat-day", selectedHeatDate], enabled: Boolean(selectedHeatDate), queryFn: () => api<TransactionPage>(`/api/transactions?date=${selectedHeatDate}&direction=expense&page=1&pageSize=100`) });
  if (isLoading || !data) return isError ? <ErrorState message={error instanceof Error ? error.message : "分析数据加载失败"} onRetry={() => refetch()} /> : <Skeleton active paragraph={{ rows: 14 }} />;
  if (customRange && (customLoading || !customData)) return customError ? <ErrorState message="自定义范围加载失败" onRetry={() => refetchCustom()} /> : <Skeleton active paragraph={{ rows: 14 }} />;
  ...
  const comparisonRows = customData?.comparisonBreakdown || data.comparisonBreakdowns[scope] || [];
  ...
  const focusedCategory = primaryRows.some((row) => row.category === requestedCategory) ? requestedCategory : primaryRows[0]?.category || "";
  const focusedSecondaryRows = focusedCategory ? secondaryRows.filter((row) => row.parent === focusedCategory) : secondaryRows;
  const contributionRows = comparisonRows.slice(0, 6);
  const budgetPaceRows = scope === "month" && !customRange && data.budget ? trendRows.map((row, index) => ({ period: row.label, cumulative: trendRows.slice(0, index + 1).reduce((sum, item) => sum + item.amount, 0), budget: data.budget!.amount * ((index + 1) / Math.max(1, trendRows.length)) })) : [];
  const budgetChartData = budgetPaceRows.flatMap((row) => [{ period: row.period, series: "实际累计支出", amount: row.cumulative }, { period: row.period, series: "预算进度", amount: row.budget }]);
  const budgetConfig: any = { data: budgetChartData, xField: "period", yField: "amount", colorField: "series", height: 280, autoFit: true, scale: { color: { range: ["#176b62", "#c99748"] } }, style: { lineWidth: 2 }, axis: { y: { labelFormatter: (value: number) => `¥${value}` } }, legend: { color: { position: "top" } }, tooltip: { title: { field: "period" }, items: [{ field: "amount", name: "金额", valueFormatter: (value: number) => money(value) }] } };
  ...
  const calendarMonthPicker = <DatePicker picker="month" allowClear={false} format="YYYY年M月" value={dayjs(calendarMonthStart)} disabledDate={(value) => Boolean(customRange && (value.format("YYYY-MM") < customRange[0].slice(0, 7) || value.format("YYYY-MM") > customRange[1].slice(0, 7)))} onChange={(value) => value && setParam("calendarMonth", value.format("YYYY-MM"))} />;
  return <div className="page-stack">
    ...
    {contributionRows.length > 0 && <Card title="分类变化贡献" extra={<Typography.Text type="secondary">与上一周期相比</Typography.Text>}><CategoryContribution rows={contributionRows} /></Card>}
    <div className={`analytics-insight-grid${budgetPaceRows.length > 0 ? " has-budget" : ""}`}>
      <Card title="预算消耗节奏" extra={data.budget ? <Typography.Text type="secondary">预算 {money(data.budget.amount)}</Typography.Text> : <Tag>未设置预算</Tag>}>
        {budgetPaceRows.length > 0 ? <><Line {...budgetConfig} /><Typography.Text type="secondary" className="chart-summary-visible">当前累计支出 {money(data.budget!.used)}，预算使用率 {(data.budget!.usageRate * 100).toFixed(1)}%。</Typography.Text></> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="尚未设置本月总预算"><Button type="primary" onClick={() => navigate(`/budgets?month=${selected.slice(0, 7)}`)}>去设置预算</Button></Empty>}
      </Card>
      <Card className="large-expense-card" title="大额支出" extra={<Typography.Text type="secondary">{currentRange}</Typography.Text>}>
        {largeError ? <ErrorState message="大额支出加载失败" onRetry={() => refetchLarge()} /> : <List className="large-expense-list" loading={largeLoading} dataSource={large?.records || []} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="当前范围没有支出" /> }} renderItem={(row, index) => <List.Item className="large-expense-item">
          <Typography.Text type="secondary" className="large-expense-rank">{index + 1}</Typography.Text>
          <div className="large-expense-main"><Typography.Text strong ellipsis={{ tooltip: row.item }}>{row.item}</Typography.Text><Typography.Text type="secondary">{row.date} · {row.category1}</Typography.Text></div>
          <Button type="link" onClick={() => navigate(`/transactions?start=${activeRange[0]}&end=${activeRange[1]}&query=${encodeURIComponent(row.item)}`)}>{money(Math.abs(row.amount))}</Button>
        </List.Item>} />}
      </Card>
    </div>
    <Card className="expense-calendar-card" title="每日支出日历">
      <div className="expense-calendar-layout">
        <aside className="expense-calendar-side">
          ...
          <div className="expense-calendar-controls"><Button icon={<ArrowLeftOutlined />} disabled={!canGoPreviousMonth} onClick={() => setParam("calendarMonth", previousCalendarMonth)} aria-label="上一个月" />{calendarMonthPicker}<Button icon={<ArrowRightOutlined />} disabled={!canGoNextMonth} onClick={() => setParam("calendarMonth", nextCalendarMonth)} aria-label="下一个月" /></div>
        ...
        <div className={`expense-calendar-pane${calendarLoading ? " loading" : ""}`}>
          {calendarError ? <ErrorState message="日历数据加载失败" onRetry={() => refetchCalendar()} /> : <ExpenseMonth year={Number(calendarMonth.slice(0, 4))} month={Number(calendarMonth.slice(5, 7)) - 1} amounts={heatAmounts} thresholds={heatThresholds} minDate={customRange?.[0]} maxDate={customRange?.[1]} onSelect={setSelectedHeatDate} mobile />}
        </div>
      </div>
    </Card>
    <Drawer className="expense-day-drawer" title={selectedHeatDate ? dayjs(selectedHeatDate).format("M月D日") : "当天支出"} placement={screens.md ? "right" : "bottom"} height={screens.md ? undefined : "72vh"} open={Boolean(selectedHeatDate)} onClose={() => setSelectedHeatDate("")} destroyOnHidden>
      <Statistic title="当天支出" value={heatAmounts.get(selectedHeatDate) || 0} precision={2} prefix="¥" />
      {heatDay && heatDay.total > heatDay.records.length && <Alert type="info" showIcon message={`当天共 ${heatDay.total} 笔，当前显示前 ${heatDay.records.length} 笔`} />}
...
function ExpenseLegend({ thresholds }: { thresholds: number[] }) {
  const labels = ["无", thresholds[0] ? `≤${money(thresholds[0])}` : "低", thresholds[1] ? `≤${money(thresholds[1])}` : "中", thresholds[2] ? `≤${money(thresholds[2])}` : "高", thresholds[2] ? `>${money(thresholds[2])}` : "最高"];
  return <Flex className="expense-legend" align="center" gap={6} wrap><Typography.Text type="secondary">支出强度</Typography.Text>{labels.map((label, level) => <Flex key={level} align="center" gap={3}><span className={`expense-legend-swatch level-${level}`} /><Typography.Text type="secondary">{label}</Typography.Text></Flex>)}</Flex>;
}
...
function ExpenseMonth({ year, month, amounts, thresholds, onSelect, minDate, maxDate, mobile = false }: { year: number; month: number; amounts: Map<string, number>; thresholds: number[]; onSelect: (date: string) => void; minDate?: string; maxDate?: string; mobile?: boolean }) {
  ...
  return <section className={`expense-month${mobile ? " mobile" : ""}`} aria-label={`${year}年${month + 1}月支出日历`}>
    ...
    <div className="expense-weekdays" role="row">{WEEKDAYS.map((weekday) => <span key={weekday} role="columnheader">{weekday}</span>)}</div>
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return <Alert type="error" showIcon message={message} action={<Button size="small" onClick={onRetry}>重试</Button>} />;
}

function ChartSummary({ rows }: { rows: { period: string; amount: number }[] }) {
  const peak = rows.reduce((highest, row) => row.amount > highest.amount ? row : highest, rows[0]);
  const total = rows.reduce((sum, row) => sum + row.amount, 0);
  return <Typography.Text type="secondary" className="chart-summary-visible">共支出 {money(total)}，最高支出出现在 {peak.period}，金额 {money(peak.amount)}。</Typography.Text>;
}

function SinglePointSummary({ row }: { row: { period: string; amount: number } }) {
  return <div className="chart-single-summary">
    <Typography.Text type="secondary">{row.period} 支出</Typography.Text>
    <Typography.Text strong>{money(row.amount)}</Typography.Text>
    <Typography.Text type="secondary">当前范围只有一个时间点，直接显示总额。</Typography.Text>
  </div>;
}

function CategoryContribution({ rows }: { rows: BreakdownChange[] }) {
  const max = Math.max(1, ...rows.map((row) => Math.abs(row.difference)));
  return <div className="category-contribution-list" aria-label="分类变化贡献">
    {rows.map((row) => <div className="category-contribution-row" key={row.category}>
      <Typography.Text strong className="category-contribution-label" ellipsis={{ tooltip: row.category }}>{row.category}</Typography.Text>
      <div className="category-contribution-track"><span className={`category-contribution-bar ${row.difference >= 0 ? "increase" : "decrease"}`} style={{ width: `${Math.max(4, Math.round(Math.abs(row.difference) / max * 100))}%` }} /></div>
      <Typography.Text strong className={row.difference > 0 ? "category-contribution-increase" : row.difference < 0 ? "category-contribution-decrease" : "category-contribution-flat"}>{row.difference > 0 ? "+" : ""}{money(row.difference)}</Typography.Text>
    </div>)}
  </div>;
}

function Metric({ title, value, trend, trendLabel, valueStyle }: { title: string; value: number | null; trend?: number | null; trendLabel?: string; valueStyle?: CSSProperties }) {
  const trendColor = trend == null || trend === 0 ? undefined : trend > 0 ? "#c65f43" : "#4d7a5a";
  return <Card><Statistic title={title} value={value ?? 0} formatter={value == null ? () => "—" : undefined} precision={value == null ? undefined : 2} prefix={value == null ? undefined : "¥"} valueStyle={valueStyle || { color: trendColor }} /><Flex gap={6} align="center">{trend != null && trend !== 0 && (trend > 0 ? <ArrowUpOutlined style={{ color: "#c65f43" }} /> : <ArrowDownOutlined style={{ color: "#4d7a5a" }} />)}<Typography.Text type="secondary">{trendLabel || ""}</Typography.Text></Flex></Card>;
}
