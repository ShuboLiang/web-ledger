...
export function AuthPage() {
  ...
  const submit = useMutation({
    mutationFn: async (values: AuthValues) => {
      ...
      return api<{ user: AuthUser }>(`/api/auth/${mode}`, {
        method: "POST",
    ...
    <main className="auth-page">
      <section className="auth-story">
        <div className="auth-brand">
          <BookTwoTone twoToneColor="#70d2bf" />
          <span>
            <b>轻账</b>
        ...
        <div className="auth-story-copy">
          ...
          <Typography.Paragraph>
            账目、预算、分类和 AI
            对话按账户独立保存。登录一次后保持长期有效，直到你主动退出。
          </Typography.Paragraph>
        </div>
        <Flex className="auth-security-note" gap={10} align="center">
          <SafetyCertificateOutlined />
          <span>密码加密保存 · 会话可随时撤销</span>
        </Flex>
      </section>
      <section className="auth-form-area">
        <Card className="auth-card">
          <Typography.Title level={3}>
            {mode === "login" ? "欢迎回来" : "创建个人账本"}
          </Typography.Title>
