import { ClearOutlined, DeleteOutlined, DownloadOutlined, EditOutlined, FilterOutlined, FolderOpenOutlined, SaveOutlined, SearchOutlined, SettingOutlined, TagsOutlined } from "@ant-design/icons";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { App, Button, Card, Checkbox, DatePicker, Drawer, Dropdown, Empty, Flex, Form, Grid, Input, List, Select, Space, Table, Tag, Typography, type MenuProps, type TableColumnsType } from "antd";
...
const columnLabels: Record<string, string> = { date: "日期", item: "项目", category: "分类", accountName: "账户", note: "备注", amount: "金额" };
...
export function TransactionsPage() {
  ...
  const [visible, setVisible] = useState<Record<string, boolean>>({ date: true, item: true, category: true, accountName: true, note: true, amount: true });
  ...
  const columns: TableColumnsType<Transaction> = [
    ...
    { key: "item", dataIndex: "item", title: "项目", width: 170, ellipsis: true, hidden: !visible.item, render: (value) => <Typography.Text strong>{value}</Typography.Text> },
    { key: "category", title: "分类", width: 180, hidden: !visible.category, render: (_, row) => <Tag color="cyan">{row.category1} · {row.category2}</Tag> },
    { key: "accountName", dataIndex: "accountName", title: "账户", width: 130, hidden: !visible.accountName, render: (value) => value || "未指定" },
    { key: "note", dataIndex: "note", title: "备注", ellipsis: true, hidden: !visible.note, render: (value) => value || <Typography.Text type="secondary">—</Typography.Text> },
  ...
  const savedFilterMenu: MenuProps = { items: [
    { key: "save", icon: <SaveOutlined />, label: hasSavedFilter ? "覆盖已保存方案" : "保存当前筛选", onClick: () => { const saved = new URLSearchParams(params); ["page", "pageSize", "sortBy", "sortOrder", "focus"].forEach((key) => saved.delete(key)); localStorage.setItem("ledger-saved-filter", saved.toString()); setHasSavedFilter(true); message.success("筛选方案已保存"); } },
    { key: "apply", icon: <FolderOpenOutlined />, label: "应用已保存方案", disabled: !hasSavedFilter, onClick: () => setParams(new URLSearchParams(localStorage.getItem("ledger-saved-filter") || "")) },
    { type: "divider" },
    { key: "remove", icon: <DeleteOutlined />, danger: true, label: "删除已保存方案", disabled: !hasSavedFilter, onClick: () => { localStorage.removeItem("ledger-saved-filter"); setHasSavedFilter(false); message.success("已删除筛选方案"); } },
  ] };
  const activeFilterCount = ["month", "accountId", "direction", "category1", "category2", "query"].filter((key) => params.has(key)).length;
  const sorter = params.get("sortBy") ? { field: params.get("sortBy"), order: params.get("sortOrder") === "asc" ? "ascend" : "descend" } : null;
  return <div className="page-stack">
    <Card><Flex vertical gap={16}><Flex wrap gap={8}>
      ...
      <Select allowClear style={{ minWidth: 130 }} placeholder="全部账户" value={params.get("accountId") || undefined} options={dictionaries?.accounts?.map((row) => ({ label: row.name, value: row.id }))} onChange={(value) => set("accountId", value || "")} />
      <Select allowClear style={{ minWidth: 120 }} placeholder="全部类型" value={params.get("direction") || undefined} options={[{ label: "支出", value: "expense" }, { label: "收入", value: "income" }]} onChange={(value) => set("direction", value || "")} />
      <Select allowClear style={{ minWidth: 140 }} placeholder="一级分类" value={params.get("category1") || undefined} options={primary.map((value) => ({ label: value, value }))} onChange={(value) => set("category1", value || "")} />
      <Select allowClear style={{ minWidth: 140 }} placeholder="二级分类" value={params.get("category2") || undefined} options={secondary.map((value) => ({ label: value, value }))} onChange={(value) => set("category2", value || "")} />
      <Input.Search ref={searchRef} allowClear prefix={<SearchOutlined />} placeholder="搜索项目、备注或分类，按 Enter 查询" value={searchValue} onChange={(event) => { setSearchValue(event.target.value); if (!event.target.value) set("query", ""); }} onSearch={(value) => set("query", value.trim())} style={{ minWidth: 260, flex: 1 }} />
    </Flex><Flex justify="space-between" wrap gap={8}><Space wrap><Button icon={<TagsOutlined />} disabled={!selectedRowKeys.length} onClick={() => setBulkOpen(true)}>批量分类{selectedRowKeys.length ? `（${selectedRowKeys.length}）` : ""}</Button><Dropdown menu={savedFilterMenu} trigger={["click"]}><Button icon={<FilterOutlined />}>筛选方案{hasSavedFilter ? " · 1" : ""}</Button></Dropdown>{activeFilterCount > 0 && <Button type="text" icon={<ClearOutlined />} onClick={() => setParams(new URLSearchParams())}>清除筛选（{activeFilterCount}）</Button>}<Button icon={<DownloadOutlined />} onClick={() => exportCsv().catch((error) => message.error(error.message))}>导出全部</Button><Dropdown menu={columnMenu} trigger={["click"]}><Button icon={<SettingOutlined />}>列设置</Button></Dropdown></Space><Typography.Text type="secondary">共 {data?.total || 0} 笔</Typography.Text></Flex></Flex></Card>
