...
export function ShoppingPage() {
  ...
  const pendingRows = view?.items.filter((item) => !item.purchased) ?? []
  const purchasedRows = view?.items.filter((item) => item.purchased) ?? []
  const sumAmount = (rows: ShoppingItem[]) =>
    Number(rows.reduce((total, row) => total + row.amount, 0).toFixed(2))
  const pendingTotal = sumAmount(pendingRows)
  const purchasedTotal = sumAmount(purchasedRows)
  return (
      ...
      >
        {view ? (
          <div className="shopping-summary" aria-live="polite">
            <Statistic
              title={`待购总价 · ${pendingRows.length} 项`}
              value={pendingTotal}
              precision={2}
              prefix="¥"
            />
