...
- “账目 → 购物计划”：保存物品、预计单价、数量及备注，自动计算各项小计、待购总价与清单总价，支持编辑、删除、标记已购买及恢复待购。清单按用户隔离，购买标记不自动生成支出。升级时先运行 `npm run db:migrate`，再构建并重启应用。
