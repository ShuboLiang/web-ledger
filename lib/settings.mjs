...
function cleanId(value, label) {
  ...
  if (!result || !/^[a-zA-Z0-9._:/-]+$/.test(result))
    throw new Error(`${label}只能包含字母、数字、点、横线、下划线、斜线或冒号`)
  return result
}
...
export function createAiSettingsStore(filePath) {
  ...
  function normalized() {
    return {
      ...
      apiType: API_TYPES.has(stored.apiType)
        ? stored.apiType
        : "openai-completions",
  ...
  return {
    ...
    save(input) {
      ...
      const next = {
        ...
        providerId:
          mode === "custom"
            ? cleanId(input.providerId, "Provider ID")
            : current.providerId,
        modelId:
          mode === "custom"
            ? cleanId(input.modelId, "Model ID")
            : current.modelId,
        baseUrl: mode === "custom" ? cleanUrl(input.baseUrl) : current.baseUrl,
        apiType:
          mode === "custom" && API_TYPES.has(input.apiType)
            ? input.apiType
            : current.apiType,
      ...
      if (mode === "custom" && !next.apiKey)
        throw new Error(
          "请填写自定义模型服务的 API Key；本地无密钥服务可填任意占位值",
        )
      stored = next
