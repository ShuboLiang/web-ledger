...
async function createLedgerAgent({ ledger, dashboard, today, piConfig }) {
  ...
  const customTools = [
      ...
      defineTool({
        ...
        label: "查询分类字典",
        description: "查询已有项目和一级分类、二级分类组合，新增时应优先精确复用。",
  ...
  const loader = new DefaultResourceLoader({
    ...
    systemPromptOverride: () => `你是“轻账”里的 AI 记账助手。今天是 ${today}。这是连续多轮对话，你必须结合此前消息理解“它”“那笔”“刚才”等指代。所有账本事实必须通过工具查询。用户表达花钱、收入、记账或记录一笔时，必须实际调用 ledger_propose_create；要求修改或删除时，必须先查询编号再实际调用对应 propose 工具。只有 propose 工具成功返回后，才能说已生成待确认操作或询问是否确认；绝对不能只在回答中虚构预览表。propose 工具不会直接写数据库，因此即使用户说“不要直接写入”，也仍应调用它。读取操作可以直接回答。周以周一到周日计算。支出金额传正数并标记 expense。优先通过字典工具复用现有项目和分类，不确定时用“其他/待分类”。回答简洁中文，并提醒用户在界面的待确认区确认操作。`,
  });
  await loader.reload();
...
async function runAgentTurn(conversation, input) {
  ...
  const prompt = notice ? `[应用状态：${notice}]\n\n${input}` : input;
  try {
...
export function createLedgerAiConversationManager({ ledger, dashboard, getToday, getConfig, ttlMs = 30 * 60 * 1000, maxConversations = 24 }) {
  ...
  async function run({ conversationId, text }) {
    ...
    try {
      if (!conversation) {
        ...
        conversation = { ...agent, busy: false, lastUsed: Date.now(), pendingOutcome: "" };
