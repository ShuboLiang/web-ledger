...
async function createLedgerAgent({ ledger, dashboard, today, piConfig }) {
  ...
  const customTools = [
      ...
      defineTool({
        ...
        execute: async (_id, params) => {
          const keyOf = (record) => [record.date, record.amount, record.direction, record.item, record.category1, record.category2].join("|");
          const existingKeys = new Set(
            context.pending
              .filter((entry) => entry.type === "create")
              .flatMap((entry) => (entry.records || []).map(keyOf)),
          );
          const records = (params.records || []).filter((record) => !existingKeys.has(keyOf(record)));
          ...
          return toolResult({ proposed: true, count: records.length, message: "已进入待确认区，尚未写入数据库" });
        },
      }),
      defineTool({
        ...
        execute: async (_id, params) => {
          ...
          const existing = context.pending.findIndex((entry) => entry.type === "update" && entry.id === params.id);
      ...
      defineTool({
        ...
        execute: async (_id, params) => {
          ...
          context.touched = true;
          context.proposals.push(proposal);
          const existing = context.pending.findIndex((entry) => entry.type === "delete" && entry.id === params.id);
          if (existing >= 0) context.pending[existing] = proposal;
          else context.pending.push(proposal);
      ...
      defineTool({
        ...
        label: "移除待确认记录",
        description: "从待确认区中移除一笔或多笔待确认记录（尚未写入账本，仅撤消待确认建议，不修改已写入的账目）。当用户要求取消、忽略或替换某笔待确认记录时调用，可以按账目编号、项目名称、金额或日期匹配。",
        parameters: Type.Object({
          id: Type.Optional(Type.Number({ minimum: 1, description: "账目编号，用于定位待确认的修改/删除记录" })),
          item: Type.Optional(Type.String({ description: "项目名称，模糊匹配要移除的记录" })),
          amount: Type.Optional(Type.Number({ minimum: 0, description: "金额，按绝对值匹配" })),
          date: Type.Optional(Type.String({ description: "日期，YYYY-MM-DD" })),
        }),
        execute: async (_id, params) => {
          context.touched = true;
          const matchesRecord = (record) => {
            if (params.id !== undefined && record.id !== undefined && record.id !== params.id) return false;
            if (params.item !== undefined && !String(record.item || "").includes(String(params.item))) return false;
            if (params.amount !== undefined && Math.abs(Math.abs(Number(record.amount)) - Number(params.amount)) > 0.001) return false;
            if (params.date !== undefined && record.date !== params.date) return false;
            return params.id !== undefined || params.item !== undefined || params.amount !== undefined || params.date !== undefined;
          };
          let removed = 0;
          const kept = [];
          ...
          return toolResult(removed > 0
            ? { discarded: true, count: removed, message: `已从待确认区移除 ${removed} 条记录` }
            : { discarded: false, count: 0, message: "没有匹配到可移除的待确认记录" });
        },
      }),
  ];
  const loader = new DefaultResourceLoader({
    ...
    systemPromptOverride: () => `你是“轻账”里的 AI 记账助手。今天是 ${today}。这是连续多轮对话，你必须结合此前消息理解“它”“那笔”“刚才”等指代。所有账本事实必须通过工具查询。用户表达花钱、收入、记账或记录一笔时，必须先调用 ledger_list_dictionaries，再实际调用 ledger_propose_create；要求查找、定位或询问某笔账目时，调用 ledger_search_transactions；要求修改或删除时，先用 ledger_search_transactions 定位（用户已提供唯一编号时可省略），再必须调用 ledger_get_transaction 核实编号与最新内容，最后调用对应 propose 工具。只有 propose 工具成功返回后，才能说已生成待确认操作或询问是否确认；绝对不能只在回答中虚构预览表。propose 工具不会直接写数据库，因此即使用户说“不要直接写入”，也仍应调用它。待确认区是当前轮会话累积的待确认记录；当用户提出新记录或要求调整时，之前尚未确认的待确认记录会保留。如果用户要取消、忽略或替换某笔待确认记录（例如把“打车”改成“网约车”），必须先调用 ledger_propose_discard 移除旧记录，再调用 ledger_propose_create 生成新记录，绝不能一边声称已移除一边又保留旧记录。读取操作可以直接回答。周以周一到周日计算。支出金额传正数并标记 expense。分类策略：优先精确复用字典中语义合适的“一级分类 + 二级分类”组合；没有合适组合时，必须依据交易内容自主拟定清晰、具体的新一级分类和新二级分类，并将其放进 ledger_propose_create 的记录中。确认执行后系统会自动创建该分类。除非交易内容确实无法判断或用户明确要求，否则禁止使用“其他/待分类”作为默认兜底。项目名称也应优先复用；没有时用交易内容创建合适的新项目。回答简洁中文，并提醒用户在界面的待确认区确认操作。`,
  });
  await loader.reload();
...
async function promptSession(conversation, prompt) {
  ...
  const unsubscribe = conversation.session.subscribe((event) => {
    if (event.type === "message_update" && event.assistantMessageEvent?.type === "text_delta") delta += event.assistantMessageEvent.delta;
  });
  try {
  ...
  if (!text && lastAssistant?.errorMessage) throw new Error(lastAssistant.errorMessage);
  if (!text) throw new Error("模型没有返回文本内容，请检查模型 ID、接口协议和 Base URL");
  return text;
}

async function runAgentTurn(conversation, input, persistedPending = [], pendingNotice = "") {
  conversation.context.proposals = [];
  conversation.context.pending = Array.isArray(persistedPending) ? structuredClone(persistedPending) : [];
  ...
  const applicationNotices = [notice, pendingNotice].filter(Boolean).join("\n\n");
  ...
  if (!conversation.context.touched && CLAIMS_GENERATION.test(message)) {
    message = await promptSession(conversation, RETRY_PROMPT);
  }
  const failedClaim = !conversation.context.touched && CLAIMS_GENERATION.test(message);
  if (failedClaim) {
    message += `\n\n> ⚠️ 系统提示：回复声称已生成待确认操作，但实际没有生成任何待确认记录。可能是模型没有正确调用记账工具，请重试或换一种说法。`;
  }
  return {
    mode: "tool-agent",
    message,
    ...
    warning: failedClaim
      ? "回复声称已生成待确认操作，但实际没有生成任何待确认记录，请重试或换一种说法。"
      : "",
  };
}
...
export function createLedgerAiConversationManager({ ledger, dashboard, getToday, getConfig, ttlMs = 30 * 60 * 1000, maxConversations = 24 }) {
  ...
  async function run({ conversationId, text, history = [], pending = [], pendingNotice = "" }) {
    ...
    try {
      ...
      const result = await runAgentTurn(conversation, input, pending, pendingNotice);
