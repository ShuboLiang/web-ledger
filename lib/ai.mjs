...
async function createLedgerAgent({
  ...
  const customTools = [
    ...
    defineTool({
      ...
      label: "查询分类与账户字典",
      description:
        "查询已有项目、一级/二级分类、启用中的付款账户（含可用额度）和标签。新增账目应优先精确复用分类和标签；指定付款账户时使用真实 accountId。用户说不记账户时把 accountId 设为 none。贷款账户不会出现在字典里，放款和还款请先查询资产负债。",
      parameters: Type.Object({}),
        ...
        records: Type.Array(
          Type.Object({
            ...
            accountId: Type.Optional(
              Type.String({
                description:
                  '付款账户编号。省略则使用默认账户；设为 "none" 表示只记消费、不改变任何账户余额。',
              }),
            ),
