...
async function runAgentTurn(conversation, input, persistedPending = []) {
...
export function createLedgerAiConversationManager({ ledger, dashboard, getToday, getConfig, ttlMs = 30 * 60 * 1000, maxConversations = 24 }) {
  ...
  /** @param {{ conversationId: unknown, text: unknown, history?: Array<{ role: string, content: string }>, pending?: any[] }} request */
  async function run({ conversationId, text, history = [], pending = [] }) {
    ...
    try {
      ...
      const result = await runAgentTurn(conversation, input, pending);
