...
async function createLedgerAgent({
  ...
  const requireShopping = () => {
    if (!shopping) throw new Error("购物计划服务不可用")
    return shopping
  }
  const shoppingMonth = (value) => {
    const month = String(value || "").trim()
    if (/^\d{4}-(0[1-9]|1[0-2])$/.test(month)) return month
    const fallback = String(today || "").slice(0, 7)
    if (/^\d{4}-(0[1-9]|1[0-2])$/.test(fallback)) return fallback
    throw new Error("请填写有效月份，格式 YYYY-MM")
  }
  const customTools = [
    defineTool({
    ...
    defineTool({
      ...
      label: "查询分类与账户字典",
      description:
        "查询已有项目、一级/二级分类、启用中的付款账户（含可用额度）和标签。新增账目应优先精确复用分类和标签；指定付款账户时使用真实 accountId。用户说不记账户时把 accountId 设为 none。默认付款账户可选，未设置时省略 accountId 即不记账户。贷款账户不会出现在字典里，放款和还款请先查询资产负债。",
      parameters: Type.Object({}),
    ...
    defineTool({
      name: "ledger_get_lending_overview",
      label: "查询人情往来",
      description:
        "查询人情往来：每个往来对象的编号、姓名、谁欠谁多少，以及每一笔未结清往来的编号、事由、待结金额和约定还款日。涉及垫付、代付、借入、借出、待报销、收款或还钱时先调用。",
      parameters: Type.Object({}),
      execute: async () => toolResult(await ledger.lendingOverview()),
    }),
    defineTool({
    ...
    defineTool({
      ...
      label: "建议修改账户",
      description:
        "修改已有账户的名称、期初可用额度、默认付款状态或启用状态。默认付款账户可以取消，不是必须存在。必须先查询资产与负债并使用真实账户编号。后来发生的提额请用校准，不要改期初。",
      parameters: Type.Object({
      ...
      execute: async (_id, params) => {
        ...
        if (changes.isDefault === true && !resultingEnabled)
          throw new Error("默认付款账户必须保持启用")
        const proposal = {
          type: "account-update",
          accountId,
          changes,
    ...
    defineTool({
      ...
      label: "建议删除未使用账户",
      description:
        "永久删除零余额且没有账单、转账、还款或校准记录的账户，包括当前默认付款账户；有历史的账户只能停用。必须先查询资产与负债。",
      parameters: Type.Object({
        accountId: Type.String(),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
    ...
    defineTool({
      name: "ledger_propose_lending_entry",
      label: "建议记一笔人情往来",
      description:
        "记一笔人情往来。kind=advance 表示我先付钱、对方欠我（垫付、借出、公司待报销），amount 填这次一共付出去多少（账单原值），其中 selfAmount 指定的「我自己那份」记成我的支出并需要给出分类，剩下的部分才挂成对方欠我的；kind=covered 表示对方替我付，会记成我的一笔支出并挂上欠对方的钱，必须给出分类；kind=borrow 表示对方把现金给我，进我的账户但不算收入。advance 和 borrow 必须提供我自己的资金账户编号（先调用 ledger_get_finance_overview 获取），covered 不需要。往来对象用姓名填写，不存在时会自动创建。",
      parameters: Type.Object({
        kind: Type.Union([
        ...
        contactName: Type.String({ description: "往来对象姓名或名称" }),
        date: Type.String({ description: "YYYY-MM-DD" }),
        amount: Type.Number({
          minimum: 0.01,
          description: "advance 填这次一共付了多少，其余类型填往来金额",
        }),
        item: Type.String({ description: "事由，例如火锅聚餐、出差机票" }),
        selfAmount: Type.Optional(
          Type.Number({
            minimum: 0,
            description: "AA 时我自己承担的金额，必须小于 amount",
          }),
        ),
        category1: Type.Optional(Type.String()),
        category2: Type.Optional(Type.String()),
        accountId: Type.Optional(
          Type.String({ description: "我自己的资金账户编号" }),
        ),
        dueDate: Type.Optional(Type.String({ description: "约定还款日" })),
        note: Type.Optional(Type.String()),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        const { overrideUserAction, ...lending } = params
        const kindText = {
          advance: "我先垫付",
          covered: "别人替我付",
          borrow: "我借入现金",
        }[params.kind]
        if (params.kind !== "covered") {
          const finance = await ledger.financeOverview()
          const account = finance.accounts.find(
            (row) => row.id === params.accountId,
          )
          if (!account || !account.enabled || account.isContact)
            throw new Error("请提供启用中的自有资金账户编号")
        }
        if (
          (params.kind === "covered" || Number(params.selfAmount) > 0) &&
          (!params.category1 || !params.category2)
        )
          throw new Error("这笔往来会产生支出，必须提供一级和二级分类")
        return stageProposal(
          { type: "lending-entry", lending, display: { kindText } },
          (entry) =>
            entry.type === "lending-entry" &&
            entry.lending?.date === params.date &&
            entry.lending?.contactName === params.contactName &&
            Number(entry.lending?.amount) === Number(params.amount),
          overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_lending_settle",
      label: "建议记录往来结算",
      description:
        "记录人情往来的收款或还钱。direction=receivable 表示收回别人欠我的钱，direction=payable 表示我还钱给别人。不指定 entryId 时，金额会从该对象最早的一笔未结清往来开始依次冲抵，支持只结一部分。必须先调用 ledger_get_lending_overview 获取真实往来对象编号，并用 ledger_get_finance_overview 获取自己的资金账户编号。",
      parameters: Type.Object({
        contactId: Type.String({ description: "往来对象编号" }),
        direction: Type.Union([
          Type.Literal("receivable"),
          Type.Literal("payable"),
        ]),
        date: Type.String({ description: "YYYY-MM-DD" }),
        amount: Type.Number({ minimum: 0.01 }),
        accountId: Type.String({ description: "我自己的资金账户编号" }),
        entryId: Type.Optional(Type.String({ description: "指定某一笔往来" })),
        note: Type.Optional(Type.String()),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        const { overrideUserAction, ...settlement } = params
        const [lending, finance] = await Promise.all([
          ledger.lendingOverview(),
          ledger.financeOverview(),
        ])
        const contact = lending.contacts.find(
          (row) => row.id === params.contactId,
        )
        if (!contact) throw new Error("往来对象不存在，请先查询人情往来")
        const account = finance.accounts.find(
          (row) => row.id === params.accountId,
        )
        if (!account || !account.enabled || account.isContact)
        ...
        return stageProposal(
          {
            type: "lending-settle",
            settlement,
            display: { contactName: contact.name, accountName: account.name },
          },
          (entry) =>
            entry.type === "lending-settle" &&
            entry.settlement?.date === params.date &&
            entry.settlement?.contactId === params.contactId &&
            Number(entry.settlement?.amount) === Number(params.amount),
          overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_get_shopping_overview",
      label: "查询购物计划",
      description:
        "查询购物计划：指定月份的收入明细、想买的东西、已买勾选、计划购买合计和剩余可买额度，以及随后几个月的摘要。购物计划只规划“这个月还能买什么”，不计入账本收支、也不扣账户。涉及购物清单、本月还能买多少、购物计划工资/奖金时先调用。未指定月份时使用今天所在月份。",
      parameters: Type.Object({
        month: Type.Optional(Type.String({ description: "YYYY-MM" })),
        horizon: Type.Optional(Type.Number({ minimum: 1, maximum: 12 })),
      }),
      execute: async (_id, params) =>
        toolResult(
          await requireShopping().overview(
            params.month ? shoppingMonth(params.month) : shoppingMonth(),
            params.horizon,
          ),
        ),
    }),
    defineTool({
      name: "ledger_propose_shopping_income_create",
      label: "建议记入购物计划收入",
      description:
        "把一笔月收入记入购物计划（工资、奖金、兼职等），用于计算这个月还能买多少。这不是账本收入，不会出现在收支统计里。未指定月份时使用今天所在月份。",
      parameters: Type.Object({
        name: Type.String({ description: "收入来源，例如工资、奖金" }),
        amount: Type.Number({ minimum: 0.01 }),
        month: Type.Optional(Type.String({ description: "YYYY-MM" })),
        note: Type.Optional(Type.String()),
        overrideUserAction: Type.Optional(Type.Boolean()),
      ...
      execute: async (_id, params) => {
        const month = shoppingMonth(params.month)
        const shoppingIncome = {
          month,
          name: String(params.name || "").trim(),
          amount: params.amount,
          note: String(params.note || "").trim(),
        }
        if (!shoppingIncome.name) throw new Error("请填写收入来源")
        return stageProposal(
          {
            type: "shopping-income-create",
            shoppingIncome,
            item: shoppingIncome.name,
            amount: shoppingIncome.amount,
            date: `${month}-01`,
            display: { month, name: shoppingIncome.name },
          },
          (entry) =>
            entry.type === "shopping-income-create" &&
            entry.shoppingIncome?.month === month &&
            entry.shoppingIncome?.name === shoppingIncome.name &&
            Number(entry.shoppingIncome?.amount) ===
              Number(shoppingIncome.amount),
          params.overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_shopping_income_update",
      label: "建议修改购物计划收入",
      description:
        "修改购物计划中的一笔月收入。必须先调用 ledger_get_shopping_overview 获取真实收入编号。只填写需要改的字段。",
      parameters: Type.Object({
        incomeId: Type.String({ description: "购物计划收入编号" }),
        name: Type.Optional(Type.String()),
      ...
      execute: async (_id, params) => {
        const current = await requireShopping().getIncome(params.incomeId)
        if (!current) throw new Error("购物计划收入不存在，请先查询购物计划")
        const changes = {}
        if (params.name !== undefined) changes.name = String(params.name).trim()
        if (params.amount !== undefined) changes.amount = params.amount
        if (params.month !== undefined)
          changes.month = shoppingMonth(params.month)
        if (params.note !== undefined) changes.note = String(params.note).trim()
        if (!Object.keys(changes).length) throw new Error("没有要修改的内容")
        return stageProposal(
          {
            type: "shopping-income-update",
            incomeId: params.incomeId,
            changes,
            item: current.name,
            amount: changes.amount ?? current.amount,
            date: `${changes.month || current.month}-01`,
            display: { month: current.month, name: current.name },
          },
          (entry) =>
            entry.type === "shopping-income-update" &&
            entry.incomeId === params.incomeId,
          params.overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_shopping_income_delete",
      label: "建议删除购物计划收入",
      description:
        "从购物计划中删除一笔月收入。必须先调用 ledger_get_shopping_overview 获取真实收入编号。不会删除账本里已经记下的收入。",
      parameters: Type.Object({
        incomeId: Type.String({ description: "购物计划收入编号" }),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        ...
        return stageProposal(
          ...
          (entry) =>
            entry.type === "shopping-income-delete" &&
            entry.incomeId === params.incomeId,
          params.overrideUserAction,
        )
      },
    ...
    defineTool({
      name: "ledger_propose_shopping_income_copy",
      label: "建议复制上月购物收入",
      description:
        "把上个月的购物计划收入复制到指定月份。目标月份必须还没有收入。必须先调用 ledger_get_shopping_overview 确认上月有收入、本月为空。未指定月份时复制到今天所在月份。",
      parameters: Type.Object({
        month: Type.Optional(
          Type.String({ description: "要复制到的月份，YYYY-MM" }),
        ),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        ...
        const previousMonth = new Date(Date.UTC(year, mon - 2, 1))
          .toISOString()
          .slice(0, 7)
        const overview = await requireShopping().overview(month)
        if (overview.incomeCount)
          throw new Error("本月已有收入，请直接改金额或添加其他收入")
        const previousOverview = await requireShopping().overview(previousMonth)
        if (!previousOverview.incomeCount)
          throw new Error(`${previousMonth} 没有收入可复制`)
        return stageProposal(
          {
            ...
            item: "复制上月购物收入",
            date: `${month}-01`,
            display: { month, from: previousMonth },
          },
          (entry) =>
            entry.type === "shopping-income-copy" && entry.month === month,
          params.overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_shopping_item_create",
      label: "建议加入购物清单",
      description:
        "把想买的东西加入指定月份的购物清单。这只是计划，不会记成支出，也不扣账户。未指定月份时使用今天所在月份。用户已经实际花掉并要记账时，请用 ledger_propose_create。",
      parameters: Type.Object({
        name: Type.String({ description: "物品名称" }),
        amount: Type.Number({ minimum: 0.01, description: "预计价格" }),
        month: Type.Optional(Type.String({ description: "YYYY-MM" })),
        note: Type.Optional(Type.String()),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        ...
        if (!shoppingItem.name) throw new Error("请填写要买的东西")
        return stageProposal(
          {
            ...
            date: `${month}-01`,
            display: { month, name: shoppingItem.name },
          },
          (entry) =>
            entry.type === "shopping-item-create" &&
            entry.shoppingItem?.month === month &&
            entry.shoppingItem?.name === shoppingItem.name &&
            Number(entry.shoppingItem?.amount) === Number(shoppingItem.amount),
          params.overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_shopping_item_update",
      label: "建议修改购物清单",
      description:
        "修改购物清单中的一件物品，包括改价、改名、换月份或勾选/取消已买。勾选已买不会记入账本支出。必须先调用 ledger_get_shopping_overview 获取真实物品编号。只填写需要改的字段。",
      parameters: Type.Object({
        itemId: Type.String({ description: "购物清单物品编号" }),
        name: Type.Optional(Type.String()),
        amount: Type.Optional(Type.Number({ minimum: 0.01 })),
        month: Type.Optional(Type.String({ description: "YYYY-MM" })),
        note: Type.Optional(Type.String()),
        purchased: Type.Optional(
          Type.Boolean({ description: "true 表示已买，false 表示待买" }),
        ),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        const current = await requireShopping().getItem(params.itemId)
        if (!current) throw new Error("购物清单项目不存在，请先查询购物计划")
        const changes = {}
        if (params.name !== undefined) changes.name = String(params.name).trim()
        if (params.amount !== undefined) changes.amount = params.amount
        if (params.month !== undefined)
        ...
        if (params.purchased !== undefined)
          changes.purchased = params.purchased === true
        if (!Object.keys(changes).length) throw new Error("没有要修改的内容")
        return stageProposal(
          {
          ...
          (entry) =>
            entry.type === "shopping-item-update" &&
            entry.itemId === params.itemId,
          params.overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_shopping_item_delete",
      label: "建议移出购物清单",
      description:
        "从购物清单移除一件物品。必须先调用 ledger_get_shopping_overview 获取真实物品编号。不会删除已经记下的账目。",
      parameters: Type.Object({
        itemId: Type.String({ description: "购物清单物品编号" }),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
      execute: async (_id, params) => {
        ...
        return stageProposal(
          ...
          (entry) =>
            entry.type === "shopping-item-delete" &&
            entry.itemId === params.itemId,
          params.overrideUserAction,
        )
      },
        ...
        records: Type.Array(
          Type.Object({
            ...
            accountId: Type.Optional(
              Type.String({
                description:
                  '付款账户编号。省略则使用默认付款账户；没有默认账户时按 "none" 处理；设为 "none" 表示只记消费、不改变任何账户余额。',
              }),
            ),
      ...
      parameters: Type.Object({
        ...
        changes: Type.Object({
          ...
          accountId: Type.Optional(
            Type.String({
              description:
                '付款账户编号。省略则保持原账户；设为 "none" 表示改为不记账户，只记消费、不改变任何账户余额。',
            }),
          ),
          tagNames: Type.Optional(Type.Array(Type.String(), { maxItems: 8 })),
        }),
      ...
      parameters: Type.Object({
        ...
        proposalType: Type.Optional(
          Type.Union([
            ...
            Type.Literal("shopping-income-create"),
            Type.Literal("shopping-income-update"),
            Type.Literal("shopping-income-delete"),
            Type.Literal("shopping-income-copy"),
            Type.Literal("shopping-item-create"),
            Type.Literal("shopping-item-update"),
            Type.Literal("shopping-item-delete"),
          ]),
        ),
        overrideUserAction: Type.Optional(
          Type.Boolean({
  ...
  const loader = new DefaultResourceLoader({
    ...
    systemPromptOverride: () => {
      ...
      const customBlock = custom
        ...
        : ""
      return `你是“轻账”里的 AI 记账助手。今天是 ${today}。可见的思考过程和最终回答都必须使用简体中文；不要在思考过程里改用英文。${mode}所有账本事实必须通过工具查询。用户表达花钱、收入、记账或记录一笔时，必须先调用 ledger_list_dictionaries，再实际调用 ledger_propose_create；要求查找、定位或询问某笔账目时，调用 ledger_search_transactions；要求修改或删除时，先用 ledger_search_transactions 定位（用户已提供唯一编号时可省略），再必须调用 ledger_get_transaction 核实编号与最新内容，最后调用对应 propose 工具。用户明确要求修改分类图标时，先调用 ledger_list_dictionaries 确认分类和当前图标，再调用 ledger_propose_category_icon_update。标签用于记录分类之外的场景、人物或目的，例如人情请客、朋友聚会；记账时优先复用字典中的标签并在 tagNames 中填写，只有明确且可长期复用的语义才自动创建新标签，每笔最多 3 个，禁止创建与消费分类同义的标签。用户要求管理标签或查询标签消费时，先调用 ledger_get_tag_overview 获取真实标签编号与状态；需要具体消费信息时继续调用 ledger_get_tag_analytics，变更时调用对应 tag propose 工具。涉及账户余额、可用额度、账户资料、转账、贷款放款、信用还款、额度校准或撤销资金移动时，必须先调用 ledger_get_finance_overview 获取真实账户编号，再调用对应的 finance propose 工具。停用账户不可继续记账、转账、还款或校准。信用/贷款账户可自由转账；贷款放款用转账（贷款账户 → 银行卡），还款用 ledger_propose_repayment（本金转账 + 可选利息支出）。撤销转账用 ledger_propose_transfer_reverse；撤销额度调整用 ledger_propose_adjustment_reverse，不要把 adjustment 当成转账撤销。涉及跟具体某个人、公司或团体之间的借出、借入、垫付、代付、AA 分摊、待报销、催收或还钱时，必须走人情往来：先调用 ledger_get_lending_overview 获取往来对象编号和未结清明细，记新往来用 ledger_propose_lending_entry，收款或还钱用 ledger_propose_lending_settle，不要用 ledger_propose_transfer 或 ledger_propose_create 手工拼凑。垫付时 amount 填这次一共付出去多少，selfAmount 填其中自己那份（必须小于 amount），只有自己那份计入支出，剩下的挂成对方欠我的；别人替我付的钱要用 kind=covered，它会同时记支出和欠款。购物计划在分析页，按月记录“这个月能拿到的钱”和“打算买的东西”，用收入减去计划购买得到剩余，不计入账本收支、也不扣账户。用户说想买、打算买、加入购物清单、规划购物、填这个月工资/奖金到购物计划、复制上月购物收入、勾选已买或从购物清单移除时，必须先调用 ledger_get_shopping_overview 获取真实编号和当月余量，再调用对应 shopping propose 工具；未指定月份时使用今天所在月份。已经实际花掉、要记进账本的消费仍用 ledger_propose_create，不要和购物计划混用。普通账户修改不能直接覆盖当前额度，实际额度不一致或提额时使用账户校准工具；期初填错才改 openingBalance。严禁把账户间转账、余额校准或偿还本金记为支出，只有利息和手续费属于支出。普通消费仍调用 ledger_propose_create；用户未说明付款账户时不要填写 accountId；若已设置默认付款账户则使用它，否则按不记账户处理。用户明确付款账户时，只能把字典中启用的真实 accountId 写入记录。用户明确说不记账户、不扣账户、别人请客、只记消费或不想记录账户时，必须把 accountId 设为 "none"，不要猜测或补默认账户。默认付款账户可以设置、取消或删除，不是必须存在。${confirmation}读取操作可以直接回答。周以周一到周日计算。支出金额传正数并标记 expense。分类策略：优先精确复用字典中语义合适的“一级分类 + 二级分类”组合；已有分类必须沿用字典中的图标，不得因普通记账而修改。没有合适组合时，必须依据交易内容自主拟定清晰、具体的新一级分类和新二级分类，并在 ledger_propose_create 的记录中同时提供语义匹配的 primaryIcon 和 secondaryIcon；记账后系统会自动创建并设置图标。${CATEGORY_ICON_GUIDE}。除非交易内容确实无法判断或用户明确要求，否则禁止使用“其他/待分类”作为默认兜底。项目名称也应优先复用；没有时用交易内容创建合适的新项目。${closing}${customBlock ? `\n${customBlock}` : ""}`
    },
  })
  await loader.reload()
...
const RETRY_PROMPT = `系统检测到你上一轮回复声称已经生成待确认操作，但实际没有任何操作进入待确认区。请立即检查用户要求，并实际调用对应的 propose 工具来生成真实操作；普通账目使用 ledger_propose_create / update / delete，账户、转账、还款、额度调整使用对应 finance propose 工具，购物计划使用对应 shopping propose 工具。如果不需要变更，请直接说明，不要声称已生成。`
const DIRECT_CLAIMS_GENERATION =
  /已(?:写入|记录|记入|记账|修改|删除|保存|新增|转账|还款|校准|调整)/
const DIRECT_RETRY_PROMPT = `系统检测到你上一轮回复声称已经完成操作，但实际账本没有变化。请根据用户要求实际调用对应的账目、finance 或 shopping propose 工具，再基于工具结果回答；如果不需要修改，请直接说明，不要声称已完成。`
...
const TOOL_LABELS = {
  ...
  ledger_get_lending_overview: "查询人情往来",
  ...
  ledger_propose_lending_entry: "建议记一笔人情往来",
  ledger_propose_lending_settle: "建议记录往来结算",
  ledger_get_shopping_overview: "查询购物计划",
  ledger_propose_shopping_income_create: "建议记入购物计划收入",
  ledger_propose_shopping_income_update: "建议修改购物计划收入",
  ledger_propose_shopping_income_delete: "建议删除购物计划收入",
  ledger_propose_shopping_income_copy: "建议复制上月购物收入",
  ledger_propose_shopping_item_create: "建议加入购物清单",
  ledger_propose_shopping_item_update: "建议修改购物清单",
  ledger_propose_shopping_item_delete: "建议移出购物清单",
...
export async function runPiLedgerDirectCommand({
  ...
  try {
    ...
    const execution = await (executeOperations
      ? executeOperations(result.proposals || [])
      : ledger.executeAiOperations(result.proposals || []))
