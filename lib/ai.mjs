...
export function pendingRecordMatches(record, params) {
  ...
  const liabilityId = String(params.liabilityId ?? "").trim()
  const hasLiabilityId = liabilityId.length > 0
  ...
  if (hasProposalType && String(record?.type || "") !== proposalType)
    return false
  if (hasLiabilityId && String(record?.liabilityId || "") !== liabilityId)
    return false
...
async function createLedgerAgent({
  ...
  const customTools = [
    ...
    defineTool({
      ...
      label: "查询资产与负债",
      description:
        "查询账户编号、账户余额、总资产、总负债、净资产、贷款分期、下一期待还和近期转账。涉及账户、转账、贷款、还款或提前结清时应先调用。",
      parameters: Type.Object({}),
    ...
    defineTool({
      ...
      label: "建议新增账户",
      description:
        "新增现金、银行卡、电子钱包、信用账户或贷款账户。负债账户的 openingBalance 填当前欠款的正数，系统会自动按负债处理。",
      parameters: Type.Object({
        name: Type.String(),
        type: Type.Union([
          Type.Literal("cash"),
    ...
    defineTool({
      ...
      label: "建议修改标签",
      description:
        "修改已有标签的名称、颜色或启用状态。必须先查询分类字典并使用真实标签编号。",
      parameters: Type.Object({
        tagId: Type.String(),
    ...
    defineTool({
      ...
      label: "建议删除标签",
      description:
        "永久删除已有标签并从历史账目解除关联；账目本身不会删除。必须先查询分类字典并使用真实标签编号。",
      parameters: Type.Object({
        tagId: Type.String(),
        overrideUserAction: Type.Optional(Type.Boolean()),
      }),
    ...
    defineTool({
      ...
      label: "建议修改账户",
      description:
        "修改已有账户的名称、默认付款状态或启用状态。必须先查询资产与负债并使用真实账户编号；不能修改账户类型或直接覆盖余额。",
      parameters: Type.Object({
    ...
    defineTool({
      ...
      label: "建议账户转账",
      description:
        "在两个已有账户之间移动资金。转账不会计入收入或支出。必须先查询资产与负债并使用真实账户编号。",
      parameters: Type.Object({
      ...
      execute: async (_id, params) => {
        ...
        if (
          finance.liabilities.some(
            (liability) =>
              liability.accountId === from.id || liability.accountId === to.id,
          )
        )
          throw new Error("分期负债请使用还款或提前结清，不能使用普通转账")
        const proposal = {
          type: "transfer",
    ...
    defineTool({
      ...
      label: "建议校准账户余额",
      description:
        "根据银行、钱包或现金的实际余额校准账户。差额不计入收入或支出。必须先查询资产与负债并使用真实、启用中的账户编号。",
      parameters: Type.Object({
        accountId: Type.String(),
        balance: Type.Number(),
        note: Type.Optional(Type.String()),
      ...
      execute: async (_id, params) => {
        ...
        if (finance.liabilities.some((row) => row.accountId === account.id))
          throw new Error("分期负债不能直接校准，请使用还款或撤销还款")
    ...
    defineTool({
      ...
      label: "建议撤销账户转账",
      description:
        "撤销一笔普通账户转账并恢复双方余额。只能使用资产负债概览中 kind=transfer 的真实资金移动编号。",
      parameters: Type.Object({
      ...
      execute: async (_id, params) => {
        ...
        if (!transfer || transfer.kind !== "transfer")
          throw new Error("普通转账不存在或不在最近资金移动中")
        return stageProposal(
          {
    ...
    defineTool({
      name: "ledger_propose_liability_create",
      label: "建议新增负债计划",
      description:
        "新增贷款、信用欠款或消费分期，并生成每月计划。fundingMode=opening 表示只登记当前欠款；deposit 表示新贷款放款到 targetAccountId。",
      parameters: Type.Object({
        name: Type.String(),
        kind: Type.Union([
          Type.Literal("loan"),
        ...
        totalInterest: Type.Optional(Type.Number({ minimum: 0 })),
        totalInstallments: Type.Number({ minimum: 1, maximum: 600 }),
        startDate: Type.String({ description: "YYYY-MM-DD" }),
        firstDueDate: Type.String({ description: "YYYY-MM-DD" }),
        fundingMode: Type.Union([
      ...
      execute: async (_id, params) => {
        const { overrideUserAction, ...liability } = params
        const proposal = { type: "liability-create", liability }
        return stageProposal(
          proposal,
          (entry) =>
            entry.type === "liability-create" &&
            entry.liability?.name === params.name,
          overrideUserAction,
        )
      },
    }),
    defineTool({
      name: "ledger_propose_liability_payment",
      label: "建议偿还一期",
      description:
        "偿还一个已有负债计划的下一期。principal 是本金，不计入支出；interest 和 fee 会计入当月支出。必须先查询资产与负债取得真实负债和付款账户编号。",
      parameters: Type.Object({
        liabilityId: Type.String(),
        date: Type.String({ description: "YYYY-MM-DD" }),
        sourceAccountId: Type.String(),
        principal: Type.Optional(Type.Number({ minimum: 0.01 })),
      ...
      execute: async (_id, params) => {
        const { liabilityId, overrideUserAction, ...payment } = params
        const finance = await ledger.financeOverview()
        const liability = finance.liabilities.find(
          (row) => row.id === liabilityId,
        )
        const source = finance.accounts.find(
          (account) => account.id === payment.sourceAccountId,
        )
        if (
          !liability ||
          ...
          throw new Error("负债不存在、已结清或没有待还分期")
        if (!source || !source.enabled || source.isLiability)
          throw new Error("请选择启用中的资产账户还款")
        if (
          payment.principal !== undefined &&
          Number(payment.principal.toFixed(2)) !==
            Number(liability.nextInstallment.principal.toFixed(2))
        )
          throw new Error("还一期必须使用计划本金；全部还清请使用提前结清")
        const proposal = {
          type: "liability-payment",
          liabilityId,
          payment,
          display: {
            liabilityName: liability?.name || liabilityId,
            sourceAccountName: source?.name || payment.sourceAccountId,
          },
        }
        return stageProposal(
          proposal,
          (entry) =>
            entry.type === "liability-payment" &&
    ...
    defineTool({
      name: "ledger_propose_liability_settlement",
      label: "建议提前结清",
      description:
        "提前结清一个已有负债。系统自动使用当前全部剩余本金、取消未来分期并保留历史；interest 和 fee 填结清当天实际利息与手续费。",
      parameters: Type.Object({
        liabilityId: Type.String(),
        date: Type.String({ description: "YYYY-MM-DD" }),
        sourceAccountId: Type.String(),
      ...
      execute: async (_id, params) => {
        const { liabilityId, overrideUserAction, ...settlement } = params
        const finance = await ledger.financeOverview()
        const liability = finance.liabilities.find(
          (row) => row.id === liabilityId,
        )
        const source = finance.accounts.find(
          (account) => account.id === settlement.sourceAccountId,
        )
        if (!liability || liability.status !== "active")
          throw new Error("负债不存在或已经结清")
        if (!source || !source.enabled || source.isLiability)
          throw new Error("请选择启用中的资产账户结清")
        const proposal = {
          ...
          display: {
            liabilityName: liability?.name || liabilityId,
            sourceAccountName: source?.name || settlement.sourceAccountId,
        ...
        return stageProposal(
          proposal,
          (entry) =>
            entry.type === "liability-settlement" &&
            entry.liabilityId === liabilityId,
          overrideUserAction,
        )
      },
    ...
    defineTool({
      name: "ledger_propose_liability_payment_reverse",
      label: "建议撤销最近还款",
      description:
        "撤销一个负债计划的最后一笔还款，同时恢复本金转账、利息费用和分期状态。必须先查询资产与负债并使用真实编号。",
      parameters: Type.Object({
        liabilityId: Type.String(),
      ...
      execute: async (_id, params) => {
        const finance = await ledger.financeOverview()
        const liability = finance.liabilities.find(
          (row) => row.id === params.liabilityId,
        )
        const payment = liability?.payments.find(
          (row) => row.id === params.paymentId,
        )
        if (!liability || !payment)
          throw new Error("还款记录不存在，请重新查询")
        if (liability.payments[0]?.id !== payment.id)
          throw new Error("只能撤销该负债的最后一笔还款")
        return stageProposal(
          {
          ...
          (entry) =>
            entry.type === "liability-payment-reverse" &&
      ...
      parameters: Type.Object({
        ...
        proposalType: Type.Optional(
          Type.Union([
            ...
            Type.Literal("liability-create"),
            Type.Literal("liability-payment"),
            Type.Literal("liability-settlement"),
          ]),
        ),
        liabilityId: Type.Optional(
          Type.String({ description: "负债计划编号" }),
        ),
        overrideUserAction: Type.Optional(
          Type.Boolean({
            description:
            ...
            const matched = pendingRecordMatches(
              {
                ...
                item:
                  entry.current?.item ||
                  entry.liability?.name ||
                  entry.account?.name ||
                  entry.tag?.name ||
                  entry.display?.tagName ||
                  entry.display?.accountName ||
                  entry.display?.liabilityName ||
                  "",
              },
              params,
            )
...
const CLAIMS_GENERATION =
  /已(?:为您|为你|帮你|为)?生成|已进入待确认区|已添加|已(?:新增|转账|还款|结清)/
const RETRY_PROMPT = `系统检测到你上一轮回复声称已经生成待确认操作，但实际没有任何操作进入待确认区。请立即检查用户要求，并实际调用对应的 propose 工具来生成真实操作；普通账目使用 ledger_propose_create / update / delete，账户、转账、负债与还款使用对应 finance propose 工具。如果不需要变更，请直接说明，不要声称已生成。`
const DIRECT_CLAIMS_GENERATION =
  /已(?:写入|记录|记入|记账|修改|删除|保存|新增|转账|还款|结清)/
...
const TOOL_LABELS = {
  ...
  ledger_propose_account_reconcile: "建议校准账户余额",
  ...
  ledger_propose_liability_create: "建议新增负债计划",
  ledger_propose_liability_payment: "建议偿还一期",
  ledger_propose_liability_settlement: "建议提前结清",
  ledger_propose_liability_payment_reverse: "建议撤销还款",
