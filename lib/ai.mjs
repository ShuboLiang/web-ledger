...
async function promptSession(conversation, prompt, onEvent) {
  let delta = ""
  let thinking = ""
...
async function runAgentTurn(
  ...
  let response = await promptSession(conversation, prompt, onEvent)
  ...
  if (!conversation.context.touched && claims.test(message)) {
    response = await promptSession(conversation, retryPrompt, onEvent)
...
export function createLedgerAiConversationManager({
  ...
  /** @param {{ conversationId: unknown, text: unknown, history?: Array<{ role: string, content: string }>, pending?: any[], pendingNotice?: string, onEvent?: (event: { type: string, data?: unknown }) => void }} request */
  async function run({
    conversationId,
    text,
    history = [],
