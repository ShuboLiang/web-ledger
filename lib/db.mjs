...
export function createLedgerDatabase(customPath) {
  const dbPath = path.resolve(
    customPath ||
      process.env.DB_PATH ||
      path.join(appRoot, "data", "ledger.sqlite"),
  )
  ...
  const insertCategory = db.prepare(
    "INSERT OR IGNORE INTO categories(category1, category2) VALUES (?, ?)",
  )
  ...
  if (count === 0 && process.env.NO_SEED !== "1") {
    const seedPath = path.resolve(
      process.env.INITIAL_LEDGER_PATH ||
        path.join(appRoot, "data", "initial-ledger.json"),
    )
  ...
  return {
    ...
    pageTransactions({
      ...
      const selectedDirection = ["expense", "income"].includes(direction)
        ? direction
        : ""
      const clauses = []
      const params = []
      ...
      if (search) {
        clauses.push(
          "(item LIKE ? OR note LIKE ? OR category1 LIKE ? OR category2 LIKE ?)",
        )
      ...
      if (minVal !== null || maxVal !== null) {
        if (selectedDirection === "expense") {
          if (minVal !== null && maxVal !== null) {
            clauses.push("amount >= ? AND amount <= ?")
            params.push(-maxVal, -minVal)
          } else if (minVal !== null) {
            clauses.push("amount <= ?")
            params.push(-minVal)
          } else {
            clauses.push("amount >= ? AND amount < 0")
            params.push(-maxVal)
          }
          ...
          if (minVal !== null && maxVal !== null) {
            clauses.push(
              "((amount >= ? AND amount <= ?) OR (amount >= ? AND amount <= ?))",
            )
            params.push(-maxVal, -minVal, minVal, maxVal)
          } else if (minVal !== null) {
            clauses.push("(amount <= ? OR amount >= ?)")
        ...
        clauses.push(
          selectedDirection === "expense" ? "amount < 0" : "amount > 0",
        )
      }
    ...
    get(id) {
      return (
        db.prepare("SELECT * FROM transactions WHERE id = ?").get(Number(id)) ||
        null
      )
    },
    dictionaries() {
      return {
        projects: db
          ...
          .map((row) => row.name),
        categories: db
          .prepare("SELECT category1, category2 FROM categories ORDER BY id")
          .all(),
      }
    },
    addMany,
