...
function renderBreakdown() {
  const source =
    state.breakdownLevel === "secondary"
      ? state.dashboard.secondaryBreakdowns
      : state.dashboard.breakdowns
  ...
  $("#donutLevelLabel").textContent =
    state.breakdownLevel === "secondary" ? "二级分类" : "一级分类"
  $("#breakdownLegend").innerHTML = rows.length
    ? rows
        .map(
          (row, index) => `
...
function setBreakdownScope(scope, scrollToChart = false) {
  ...
  $$("[data-summary-scope]").forEach((card) =>
    card.classList.toggle("featured", card.dataset.summaryScope === scope),
  )
  $$("#breakdownTabs button").forEach((button) =>
  ...
  if (scrollToChart) {
    ...
    requestAnimationFrame(() =>
      $(".breakdown-panel").scrollIntoView({
        behavior: "smooth",
        block: "center",
      }),
    )
...
function renderRecords() {
  ...
  $("#recordRows").innerHTML = state.records.length
    ? state.records
        .map(
          (row) => `
    <tr>
      ...
      <td class="row-actions"><button class="row-action" data-edit="${row.id}" title="编辑" aria-label="编辑 ${escapeHtml(row.item)}">✎</button><button class="row-action danger-text" data-delete="${row.id}" title="删除" aria-label="删除 ${escapeHtml(row.item)}">×</button></td>
        ...
        .join("")
    : `<tr><td colspan="6" class="empty">${state.recordQuery || state.recordCategory ? "没有符合筛选条件的账目" : "这个月还没有账目"}</td></tr>`
  $("#pageInfo").textContent =
    `第 ${state.recordPage} / ${state.recordTotalPages} 页`
  $("#previousPage").disabled = state.recordPage <= 1
  $("#nextPage").disabled = state.recordPage >= state.recordTotalPages
  $("#clearRecordFilters").classList.toggle(
    "hidden",
    !state.recordQuery && !state.recordCategory,
  )
}

function renderDashboardRecent() {
  const container = $("#dashboardRecent")
  container.innerHTML = state.recentRecords.length
    ? state.recentRecords
        .map(
          (row) => `
    <div class="recent-row">
      ...
      <strong class="recent-amount ${row.amount < 0 ? "expense" : "income"}">${row.amount < 0 ? "−" : "+"}${money(Math.abs(row.amount))}</strong>
        ...
        .join("")
    : '<div class="empty">还没有账目，先记下第一笔吧</div>'
...
function renderMarkdown(markdown) {
  ...
  while (index < lines.length) {
    ...
    if (
      ...
      output.push(
        `<div class="markdown-table-wrap"><table><thead><tr>${headers.map((cell) => `<th>${renderInlineMarkdown(cell)}</th>`).join("")}</tr></thead><tbody>${rows.map((row) => `<tr>${headers.map((_, cellIndex) => `<td>${renderInlineMarkdown(row[cellIndex] || "")}</td>`).join("")}</tr>`).join("")}</tbody></table></div>`,
      )
      continue
    }
    if (/^>\s?/.test(line)) {
      ...
      output.push(
        `<blockquote>${quote.map(renderInlineMarkdown).join("<br>")}</blockquote>`,
      )
      continue
    }
...
function renderDictionaries() {
  $("#projectList").innerHTML = state.dictionaries.projects
    .map((value) => `<option value="${escapeHtml(value)}"></option>`)
    .join("")
  const first = [
    ...new Set(state.dictionaries.categories.map((row) => row.category1)),
  ]
  const second = [
    ...new Set(state.dictionaries.categories.map((row) => row.category2)),
  ]
  $("#category1List").innerHTML = first
    .map((value) => `<option value="${escapeHtml(value)}"></option>`)
    .join("")
  $("#category2List").innerHTML = second
    .map((value) => `<option value="${escapeHtml(value)}"></option>`)
    .join("")
  $("#recordCategory").innerHTML =
...
async function selectHistoryYear(year) {
  ...
  if (
    ...
    throw new Error("请输入 1900 至 9999 之间的年份")
...
function renderConversationMessage(message) {
  if (message.role === "event")
    return `<div class="conversation-event"><span>✓</span>${escapeHtml(message.content)}</div>`
  if (message.role === "thinking")
    return `<div class="conversation-message assistant"><div class="message-avatar">✦</div><div class="message-content"><small>AI 助手</small><div class="typing-indicator"><i></i><i></i><i></i><span>正在思考并查询账本</span></div></div></div>`
...
function renderPreview(warning = state.aiWarning) {
  ...
  const messages = state.aiBusy
    ? [...state.aiMessages, { role: "thinking", content: "" }]
    : state.aiMessages
  if (!messages.length && !state.proposals.length)
    return panel.classList.add("hidden")
  panel.classList.remove("hidden")
  const mutations = state.proposals.filter(
    (proposal) => proposal.type !== "create",
  )
  panel.innerHTML = `
    ...
    ${
      ...
      ${mutations
        ...
        <div class="proposal-card"><div><strong>删除：${escapeHtml(proposal.current?.item || `账目 #${proposal.id}`)}</strong><small>${proposal.current?.date || ""} · ${money(Math.abs(proposal.current?.amount || 0))}${proposal.reason ? ` · ${escapeHtml(proposal.reason)}` : ""}</small></div><span class="proposal-badge">删除</span></div>`
            : `
        <div class="proposal-card"><div><strong>修改：${escapeHtml(proposal.current?.item || `账目 #${proposal.id}`)}</strong><small>${escapeHtml(
          Object.entries(proposal.changes || {})
            .map(([key, value]) => `${key}: ${value}`)
            .join("；"),
        )}</small></div><span class="proposal-badge">修改</span></div>`,
        )
        .join("")}
...
async function parseAiText() {
  ...
  try {
    const result = await api("/api/ai/command", {
      method: "POST",
      body: JSON.stringify({ text, conversationId: state.conversationId }),
    })
    state.proposals = result.proposals || []
    state.preview = state.proposals
      .filter((proposal) => proposal.type === "create")
      .flatMap((proposal) => proposal.records || [])
    state.aiMessages.push({
      role: "assistant",
      content: result.message || "AI 已完成分析。",
    })
    ...
    state.aiMessages.push({
      role: "assistant",
      content: `抱歉，这条消息处理失败：${error.message}`,
    })
...
async function reportAiOutcome(outcome) {
  try {
    await api(
      `/api/ai/conversations/${encodeURIComponent(state.conversationId)}/outcome`,
...
async function newAiConversation({ remote = true, notify = true } = {}) {
  ...
  if (remote) {
    try {
      await api(`/api/ai/conversations/${encodeURIComponent(previousId)}`, {
        method: "DELETE",
      })
    } catch {}
  }
  ...
  return $$(".preview-row").map((row) => {
    ...
    $$("[data-field]", row).forEach((input) => {
      result[input.dataset.field] =
        input.dataset.field === "amount" ? Number(input.value) : input.value
    })
    return result
  })
}
...
async function commitPreview() {
  try {
    ...
    await api("/api/ai/execute", {
      method: "POST",
      body: JSON.stringify({ proposals, conversationId: state.conversationId }),
    })
    state.preview = []
    state.proposals = []
    state.aiMessages.push({
      role: "event",
      content: `已确认并执行 ${proposals.length} 项账目操作`,
    })
    renderPreview()
    await refresh()
...
function openManualDialog(record = null) {
  ...
  form.elements.date.value =
    record?.date ||
    state.dashboard?.anchor ||
    new Date().toISOString().slice(0, 10)
  form.elements.direction.value = record
    ? record.amount > 0
      ? "income"
      : "expense"
    : "expense"
...
function openDeleteDialog(record) {
  ...
  $("#deleteRecordSummary").textContent =
    `${record.date} · ${record.item} · ${money(Math.abs(record.amount))}。删除后相关统计会自动重算。`
...
async function confirmDeleteRecord() {
  ...
  try {
    await api(`/api/transactions/${state.pendingDeleteId}`, {
      method: "DELETE",
    })
    closeDeleteDialog()
    await refresh()
...
function toggleCustomModelFields() {
  $("#customModelFields").classList.toggle(
    "hidden",
    $("#aiModeSelect").value !== "custom",
  )
}
...
async function openAiSettings() {
  try {
    ...
    form.elements.apiKey.placeholder = settings.hasApiKey
      ? "已配置；留空则保持不变"
      : "请输入 API Key"
    $("#settingsStatus").textContent = settings.hasApiKey
      ? "当前已保存 API Key（完整内容不会返回网页）"
      : "当前未保存 API Key"
...
async function saveAiSettings(event) {
  ...
  try {
    ...
    $("#settingsAiStatus").textContent =
      saved.hasApiKey || saved.mode === "default" ? "配置可用" : "等待配置"
...
document.addEventListener("click", async (event) => {
  ...
  if (scopeButton) {
    ...
    if (parent.id === "breakdownTabs")
      setBreakdownScope(scopeButton.dataset.scope)
    if (parent.id === "trendTabs") {
      state.trendScope = scopeButton.dataset.scope
      renderTrend()
    }
  }
  const levelButton = event.target.closest("[data-level]")
  if (levelButton) {
    $$("button", levelButton.parentElement).forEach((button) =>
  ...
  if (removePreview) {
    state.preview.splice(Number(removePreview.dataset.removePreview), 1)
    renderPreview()
  }
  const editButton = event.target.closest("[data-edit]")
  if (editButton) {
    const record = state.records.find(
      (row) => row.id === Number(editButton.dataset.edit),
    )
  ...
  if (deleteButton) {
    const record = state.records.find(
      (row) => row.id === Number(deleteButton.dataset.delete),
    )
  ...
  if (monthShift) {
    ...
    const shifted = new Date(
      Date.UTC(year, month - 1 + Number(monthShift.dataset.monthShift), 1),
    )
  ...
  if (dayShift) {
    shiftHistoryDate(dayShift.dataset.dayShift).catch((error) =>
      toast(error.message, true),
    )
  }
})
...
$("#historyDate").addEventListener("change", (event) =>
  selectHistoryDate(event.target.value).catch((error) =>
    toast(error.message, true),
  ),
)
$("#applyHistoryYear").addEventListener("click", () =>
  selectHistoryYear($("#historyYear").value).catch((error) =>
    toast(error.message, true),
  ),
)
$("#historyYear").addEventListener("keydown", (event) => {
  if (event.key === "Enter")
    selectHistoryYear(event.target.value).catch((error) =>
      toast(error.message, true),
    )
})
$("#showLatestStats").addEventListener("click", () =>
  showLatestStatistics().catch((error) => toast(error.message, true)),
)
$("#recordMonth").addEventListener("change", (event) =>
  selectRecordMonth(event.target.value).catch((error) =>
    toast(error.message, true),
  ),
)
...
$("#aiText").addEventListener("keydown", (event) => {
  if ((event.ctrlKey || event.metaKey) && event.key === "Enter") parseAiText()
})
$$("[data-summary-scope]").forEach((card) =>
...
const WORKSPACES = {
  dashboard: {
    ...
    title: "财务概览",
    subtitle: "快速掌握关键支出、分类结构与最近账目",
  },
  transactions: {
    eyebrow: "TRANSACTION LEDGER",
    title: "账目明细",
    subtitle: "按月份、分类与关键词检索全部账目",
  },
  analytics: {
    eyebrow: "FINANCIAL ANALYTICS",
    title: "统计分析",
    subtitle: "跨日期、周、月和年份回看支出趋势与占比",
  },
  ai: {
    eyebrow: "AI BOOKKEEPING",
    title: "AI 助手",
    subtitle: "通过多轮自然语言对话查询和管理账目",
  },
  settings: {
    eyebrow: "APPLICATION SETTINGS",
    title: "系统设置",
    subtitle: "管理模型服务、数据能力与安全选项",
  },
}
...
function showWorkspace(requested) {
  ...
  $$(".workspace-page").forEach((page) =>
    page.classList.toggle("active", page.dataset.workspace === id),
  )
...
Promise.all([api("/api/health"), refresh()])
  .then(([health]) => {
    ...
    $("#settingsAiStatus").textContent = health.aiConfigured
      ? "配置可用"
      : "需要完成模型配置"
  })
